...
async def get_treino(treino_id: str, current_user: dict = Depends(get_current_user)):
    ...
    presencas_count = await db.presencas.count_documents(
        {"treino_id": treino_id, "presente": True}
    )
...
async def list_partidas(
    ...
    for partida in partidas:
        partida["resultado"] = calcular_resultado(
            partida["gols_clube"],
...
async def create_partida(partida: PartidaCreate, current_user: dict = Depends(get_current_user)):
    ...
    partida_dict["resultado"] = calcular_resultado(
        partida.gols_clube,
        partida.gols_adversario
    )
...
async def update_partida(partida_id: str, partida: PartidaCreate, current_user: dict = Depends(get_current_user)):
    ...
    update_data["resultado"] = calcular_resultado(
        partida.gols_clube,
        partida.gols_adversario
    )
...
async def get_dashboard_charts(ano: int = 2024, current_user: dict = Depends(get_current_user)):
    ...
    for mes in range(1, 13):
        treinos_mensal.append({
            "mes": mes,
            "total": treinos_dict.get(mes, 0)
        })
...
async def export_excel(
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=ecp_{tipo}{periodo}.xlsx"
        }
    )
...
async def export_pdf(
    ...
    title = Paragraph(
        f"<b>E.C.P - Relatório de {tipo.capitalize()}{periodo_titulo}</b>",
        styles["Title"]
    )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=ecp_{tipo}.pdf"
        }
    )
